...
export default function App() {
  ...
  function openConfig() {
    ...
    setNewTienda(''); setNewEmpleada('')
    setConfigMsg(''); setShowConfig(true)
  }
  ...
  async function resetMapping() {
    try {
      ...
      setSavedMapping(null); setMapping(null); setRawRows([]); setVentasPorId({}); setStep(0); setShowConfig(false)
  ...
  async function addTienda() {
    ...
    try {
      await supabase.from('tiendas').insert({ nombre, activa:true, venta_ant:80000, crec_obj:0.05 })
      const cfg = await loadConfig(); setConfig(cfg); setEditingTiendas(cfg.tiendas.map(t=>({...t})))
      setNewTienda(''); setMsg('Local "'+nombre+'" añadido.')
    } catch(e) { setMsg('Error: '+e.message, false) }
  }
  ...
  async function deleteTienda(t) {
    ...
    try {
      ...
      const cfg = await loadConfig(); setConfig(cfg); setEditingTiendas(cfg.tiendas.map(x=>({...x})))
      setMsg('Local "'+t.nombre+'" eliminado.')
    } catch(e) { setMsg('Error: '+e.message, false) }
  }

  async function saveTiendas() {
    try {
      await Promise.all(editingTiendas.map(t => supabase.from('tiendas').update({ nombre: t.nombre.trim() }).eq('id', t.id)))
      const cfg = await loadConfig(); setConfig(cfg); setEditingTiendas(cfg.tiendas.map(t=>({...t})))
      setMsg('Nombres de locales guardados.')
    } catch(e) { setMsg('Error: '+e.message, false) }
  }
  ...
  async function addEmpleada() {
    ...
    try {
      await supabase.from('empleadas').insert({ nombre, activa:true })
      const cfg = await loadConfig(); setConfig(cfg); setEditingEmpleadas(cfg.empleadas.map(e=>({...e})))
      setNewEmpleada(''); setMsg('Empleada "'+nombre+'" añadida.')
    } catch(e) { setMsg('Error: '+e.message, false) }
  }
  ...
  async function deleteEmpleada(emp) {
    ...
    try {
      await supabase.from('empleadas').delete().eq('id', emp.id)
      const cfg = await loadConfig(); setConfig(cfg); setEditingEmpleadas(cfg.empleadas.map(x=>({...x})))
      setMsg('Empleada "'+emp.nombre+'" eliminada.')
    } catch(e) { setMsg('Error: '+e.message, false) }
  }
  ...
  function cargarHorariosDesdeExcel(file) {
    ...
    reader.onload = (e) => {
      try {
        ...
        setMsg(nuevosHorarios.length+' registros de horas cargados.')
  ...
  async function procesarYContinuar(rows, map) {
    ...
    try {
      ...
      setVentasPorId(vpi); setNoMatchTiendas(noMatch)
      await saveVentasMes(mes, vpi)
    ...
    <div className="app">
      ...
      {showConfig && (
        <div style={S.configPanel}>
          ...
          <div style={S.section}>
            ...
            {savedMapping ? <div style={{color:'#9FE1CB',fontSize:11,marginBottom:8}}>Guardado — Sucursal: <b>{savedMapping.col_sucursal}</b> · Total: <b>{savedMapping.col_total}</b></div>
              : <div style={{color:'#F09595',fontSize:11,marginBottom:8}}>Sin mapeo guardado</div>}
          ...
          <div style={S.section}>
            ...
            <p style={{color:'#aaa',fontSize:11,marginBottom:8}}>Los nombres deben coincidir exactamente con la columna Sucursal de Rapifac.</p>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:5,marginBottom:8}}>
              {editingTiendas.map((t,i)=>(
                <div key={t.id} style={{display:'flex',gap:4,alignItems:'center'}}>
                  ...
                  <button onClick={()=>deleteTienda(t)} style={{...S.btnSm,background:'#450a0a',color:'#fca5a5',padding:'5px 8px',flexShrink:0}} title="Eliminar">✕</button>
                </div>
              ))}
            </div>
          ...
          <div style={S.section}>
            <strong style={{color:'#fff',fontSize:12,display:'block',marginBottom:4}}>Empleadas ({editingEmpleadas.length})</strong>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:5,marginBottom:8}}>
              {editingEmpleadas.map((e)=>(
                <div key={e.id} style={{display:'flex',gap:4,alignItems:'center'}}>
                  ...
                  <button onClick={()=>deleteEmpleada(e)} style={{...S.btnSm,background:'#450a0a',color:'#fca5a5',padding:'3px 7px',flexShrink:0,fontSize:10}} title="Eliminar">✕</button>
                </div>
              ))}
            </div>
            <div style={{display:'flex',gap:6}}>
              <input value={newEmpleada} placeholder="Nombre de la nueva empleada..." style={{...S.input,flex:1}} onChange={e=>setNewEmpleada(e.target.value)} onKeyDown={e=>e.key==='Enter'&&addEmpleada()}/>
      ...
      {step===0 && (
        <div className="panel">
          ...
          {savedMapping ? <div className="info-card purple">Mapeo guardado — Sucursal: <b>{savedMapping.col_sucursal}</b>. Para cambiar columnas usa ⚙ Config → Resetear mapeo.</div>
          ...
          <div className="card">
            ...
            {noMatchTiendas.length>0 && (
              <div className="info-card amber" style={{marginBottom:10}}>
                ⚠ Sin coincidencia: <strong>{noMatchTiendas.join(', ')}</strong>
                <span style={{display:'block',fontSize:11,marginTop:2}}>Usa ⚙ Config → edita los nombres de locales para que coincidan.</span>
              </div>
            )}
          ...
          <div className="card">
            ...
            <div style={{display:'flex',gap:16,marginBottom:14,flexWrap:'wrap',alignItems:'flex-start',padding:'12px',background:'rgba(79,70,229,0.08)',borderRadius:8,border:'1px solid rgba(79,70,229,0.2)'}}>
              <div>
                ...
                <div style={{color:'#9CA3AF',fontSize:10,marginTop:4}}>Formato: 1ª col = Empleada, resto = nombre de tienda</div>
              </div>
              <div style={{color:'#6B7280',alignSelf:'center',fontSize:11}}>— o —</div>
              <div style={{color:'#9CA3AF',fontSize:12}}>
                <div style={{fontWeight:600,marginBottom:4,color:'#ccc'}}>✏ Opcion 2: Ingresa manualmente</div>
                Edita directamente la tabla de abajo.
              </div>
            </div>
